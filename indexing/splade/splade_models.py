...
class Splade_inference(torch.nn.Module):
    """SPLADE model
    """
    ...
    def forward(self, **tokens):
        with torch.cuda.amp.autocast() if self.fp16 else NullContextManager():
            ...
            if self.agg == "sum":
                lexical_reps = torch.sum(torch.log(1 + torch.relu(out)), dim=1)
            else:
                lexical_reps, max_indices = torch.max(torch.log(1 + torch.relu(out)), dim=1)
