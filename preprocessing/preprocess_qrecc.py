...
def gen_qrecc_passage_collection(input_passage_dir, output_file, pid2rawpid_path):
    ...
    logger.info("#totoal passages = {}".format(pid)) # 54573064
...
def gen_qrecc_qrel_for_search_query(input_test_file, output_qrel_file, pseudo_doc):
    context = True
    QR = False
    key_name = None
    model_name = "mistral"
    ...
    if context:
        ...
        key_name = model_name + "_generated_query"
    ...
    qid2pospid = {}
    if pseudo_doc:
        with open("/media/nvme/fengran/QReCC/train_with_pseudo_doc.json") as f:
            pseudo_data = f.readlines()
        for line in pseudo_data:
            record = json.loads(line)
            qid2pospid[record["sample_id"]] = record["pseudo_rel_passage_id"]
    else:
        with open("/media/nvme/fengran/QReCC/test_with_doc.json") as f:
            pseudo_data = f.readlines()
        for line in pseudo_data:
            record = json.loads(line)
            if len(record["pos_docs_pids"]) > 0:
                qid2pospid[record["sample_id"]] = int(record["pos_docs_pids"][0])
    ...
    with open(output_qrel_file, "w") as f:
        for line in tqdm(data):
            ...
            #    continue
            generated_query = line[key_name]
            query_nums = len(generated_query)
            sample_id = line["sample_id"]
            #for key in line["pos_docs_pids"]:
                #pid = int(key)
            try:
                pid = int(qid2pospid[line["sample_id"]])
            except:
                continue
            for idx in range(query_nums):
                qid = sample_id + '_' + str(idx + 1)
                f.write("{}\t{}\t{}\t{}".format(qid, 0, pid, 1))
                f.write('\n')
...
if __name__ == "__main__":
    input_passage_dir = "QReCC/collection-paragraph"
    output_file = "QReCC/qrecc_collection.tsv"
    pid2rawpid_path = "QReCC/pid2rawpid.pkl"
    ...
    pseudo_doc = False
    input_test_file = "mistral_test_with_doc_newcontext_search_q_new.json"
    output_qrel_file = "mistral_test_qrecc_qrel_newcontext.tsv"
    gen_qrecc_qrel_for_search_query(input_test_file, output_qrel_file, pseudo_doc)
    
    qrecc_collection_path = "QReCC/qrecc_collection.tsv"
    train_inputfile = "QReCC/test.json"
    train_outputfile_with_doc = "QReCC/test_with_doc.json"
