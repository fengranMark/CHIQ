...
def run_bm25(args):
    ...
    for chunk_id in range(len(qid_chunks)):
        ...
        with open(oj(args.retrieval_output_path, "best_llama_train_pseudo_doc_2.trec.{}".format(chunk_id)), "w") as f:
            for qid in qid_chunks[chunk_id]:
                ...
                for i, item in enumerate(hits[qid]):
                    ...
                    f.write('\n')
                #if ori_qid not in qid2validgenquery:
                #    qid2validgenquery[ori_qid] = []
                #else:
                #    qid2validgenquery[ori_qid].append(qid2genquery[qid])

        if not args.not_perform_evaluation:
            res, mrr_list, ndcg_3_list, recall_100_list = print_res(oj(args.retrieval_output_path, "best_llama_train_pseudo_doc_2_.trec.{}".format(chunk_id)), args.search_q_gold_qrel_file_path, args.rel_threshold)
            #try:
            #    assert sum(query_lenseg) == len(mrr_list)
            #except:
            #    breakpoint()
            reorder_gen_query_search(args, ndcg_3_list, qid2genquery)
        print("{} chunk search OK!".format(chunk_id))

def reorder_gen_query_search(args, score_list, qid2genquery):
    with open(args.test_file_path, "r") as f:
        data = f.readlines()
    ...
    with open(args.test_reoreder_file_path, "w") as g:
        for line in tqdm(data):
            line = json.loads(line)
            qid = line["sample_id"]
            #if qid.split("_") not in qid2validgenquery:
            #    breakpoint()
            #    continue
            query2score = {}
            reorder_list = []
            ...
            #scores = score_list[cur: cur + query_lenseg[idx]]
    ...
    for chunk_id in range(len(qid_chunks)):
        ...
        with open(oj(args.retrieval_output_path, "best_llama_train_pseudo_doc_0.trec.{}".format(chunk_id)), "w") as f:
            for qid in qid_chunks[chunk_id]:
                if qid not in hits:
                    print("{} not in hits".format(qid))
                    continue
        ...
        if not args.not_perform_evaluation:
            res, _, _, _ = print_res(oj(args.retrieval_output_path, "best_llama_train_pseudo_doc_0.trec.{}".format(chunk_id)), args.gold_qrel_file_path, args.rel_threshold)
...
def print_res(run_file, qrel_file, rel_threshold):
    ...
    for key in qid2genquery:
        if key not in runs:
            bad_qid.append(key)

    print(bad_qid) # ['QReCC-Train_723_4_2', 'QReCC-Train_1393_2_3', 'QReCC-Train_1765_7_4', 'QReCC-Train_2184_5_2']
    # pytrec_eval eval 
    ...
    return res, mrr_list, ndcg_3_list, recall_100_list

def get_args():
    parser = argparse.ArgumentParser()

    parser.add_argument("--index_dir_path", type = str, default="/media/nvme/fengran/index/bm25_qrecc")
    parser.add_argument("--test_file_path", type = str, default="/media/nvme/fengran/QReCC/mistral_test_with_doc_newcontext_search_q_new.json")
    parser.add_argument("--test_reoreder_file_path", type = str, default="/media/nvme/fengran/QReCC/mistral_test_with_doc_newcontext_search_q_reorder.json")
    parser.add_argument("--search_q_gold_qrel_file_path", type=str, default="/media/nvme/fengran/QReCC/mistral_test_qrecc_qrel_newcontext.tsv")
    parser.add_argument("--gold_qrel_file_path", type=str, default="/media/nvme/fengran/QReCC/qrecc_qrel.tsv")
    ...
    parser.add_argument("--bm25_k1", type=float, default=0.82)   # 0.82 for qrecc, 0.9 for topiocqa
    parser.add_argument("--bm25_b", type=float, default=0.68)    # 0.68 for qrecc, 0.4 for topiocqa
    parser.add_argument("--rel_threshold", type=int, default=1)
    parser.add_argument("--split_num_chunk", type=int, default=1)

    parser.add_argument("--retrieval_output_path", type=str, default="/media/nvme/fengran/output/bm25_qrecc")
